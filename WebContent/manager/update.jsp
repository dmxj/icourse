...
<%
	if(session.getAttribute("lecid") == null){
		response.sendRedirect("index.jsp");
	}
	Long lid = (Long)session.getAttribute("lecid");
	Lecture lecture = lectureApi.getLectureById(lid);
	if(lecture == null){
		response.sendRedirect("index.jsp");
	}
%>
...
<form action="<%=request.getContextPath() %>/manager/lectureServlet" method="post" name="updateForm">
  ...
  <fieldset class="form-group">
    ...
    <input type="text" class="form-control date-time-picker" name="week" id="week" placeholder="choose week" value="<%=lecture.getWeek()%>">
    <small class="text-muted">选择上课时间</small>
  </fieldset>
  <fieldset class="form-group">
    <label for="exampleInputEmail1">Unit:</label>
    <input type="text" class="form-control" name="unit" id="unit" placeholder="填写章节名称" value="<%=lecture.getUnit()%>">
  </fieldset>
  <fieldset class="form-group">
    <label for="exampleInputEmail1">Demo Projects:</label>
    <input type="text" class="form-control" name="demo" id="demo" placeholder="填写示例工程链接" value="<%=lecture.getDemoProject()%>">
  </fieldset>
  <fieldset class="form-group">
	<input type="button" value="保存修改" id="submitBtn" class="btn btn-success"  />
  </fieldset>
</form>
</div>
</div>		
